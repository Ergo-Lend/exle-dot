...
import lendcore.components.errors.failedTxException
...
import lendcore.core.SingleLender.Ergs.boxes.registers.{BorrowerRegister, FundingInfoRegister, LendingProjectDetailsRegister}
import lendcore.tools.runners.ExplorerRunner.{walletAddress, writeToFile}
...
case class LendInitiationRunner(lendInitiationDetails: LendInitiationDetails) {
  ...
  def handleProxyMerge(client: Client, explorer: LendBoxExplorer): Unit = {
    System.out.println(s"Getting lendcore.components.boxes for ${paymentAddress.toString}")
...
object LendCreationRunner {
  ...
  def handleLendInitiation(client: Client, explorer: LendBoxExplorer): Unit = {
    client.getClient.execute(ctx => {
      ...
      System.out.println(s"Getting lendcore.components.boxes for ${paymentAddressString}")
      ...
      val fundingInfoRegister = new FundingInfoRegister(
        ...
        deadlineHeight = deadlineHeightRecorded,
        interestRatePercent = interestRate,
        repaymentHeightLength = repaymentHeightRecorded)
