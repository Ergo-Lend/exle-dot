...
import lendcore.core.SingleLender.Ergs.boxes.SingleLenderRepaymentBox
import lendcore.core.SingleLender.Ergs.boxes.registers.{SingleAddressRegister, SingleLenderRegister}
...
object RepaymentRunner {
  ...
  def handleRefundProxy(client: Client): Unit = {
    client.getClient.execute(ctx => {
      ...
      System.out.println(s"Getting lendcore.components.boxes for ${proxyPaymentAddress}")
