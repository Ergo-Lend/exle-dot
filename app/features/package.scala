...
package object features {
  def getRequestBodyAsString(request: Request[Json], key: String, errorMessage: String = ""): String = {
    val processedErrorMessage = processErrorMessage(key, errorMessage)
    request.body.hcursor.downField(key).as[String].getOrElse(throw new Throwable(processedErrorMessage))
  }

  def getRequestBodyAsLong(request: Request[Json], key: String, errorMessage: String = ""): Long = {
    val processedErrorMessage = processErrorMessage(key, errorMessage)
