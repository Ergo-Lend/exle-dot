...
object ErgoPayUtils {
   ...
   */
  def getReducedSendTx(amountToSend: Long,
                       sender: Address,
                       recipient: Address,
                       isMainNet: Boolean): ReducedTransaction = {
   ...
   * @return ReducedTx for sender to sign
   */
  def getReducedSendTokensTx(tokensToSpend: java.util.List[ErgoToken],
                             sender: Address,
                             recipient: Address,
                             isMainNet: Boolean): ReducedTransaction = {
    val networkType: NetworkType = if (isMainNet) NetworkType.MAINNET else NetworkType.TESTNET;
  ...
  def isMainNetAddress(address: String): Boolean = {
    try {
      ...
      case e: Throwable => throw new IllegalArgumentException("Invalid address: " + address);
    }
  }
