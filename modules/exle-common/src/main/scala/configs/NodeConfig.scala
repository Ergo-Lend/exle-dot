...
import configs.NodeConfig.networkType
...
object NodeConfig extends ConfigHelper {
  ...
  lazy val SystemNodeConfig: NodeConfig = GetNodeConfig.get()
}

object MainNetNodeConfig extends ConfigHelper {
  lazy val nodeUrl: String = readKey(s"node.MAINNET.url").replaceAll("/$", "")
  lazy val explorerUrl: String = readKey(s"explorer.MAINNET.url").replaceAll("/$", "")
...
object TestNetNodeConfig extends ConfigHelper {
  ...
  lazy val explorerUrl: String = readKey(s"explorer.TESTNET.url").replaceAll("/$", "")
  lazy val explorerFront: String =
    readKey(s"explorer.TESTNET.front").replaceAll("/$", "")
}

case class NodeConfig(nodeUrl: String, explorerUrl: String, explorerFront: String)

object GetNodeConfig {
  def get(isMainNet: Boolean = (networkType == NetworkType.MAINNET)): NodeConfig = {
    if (isMainNet) {
      NodeConfig(
