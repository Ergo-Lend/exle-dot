...
class SLTLendToRepaymentTxSpec extends ErgoTestBase {
  "Lend to Repayment Tx" should {
    ...
    val wrappedSLTLendBox: SLTLendBox = createWrappedSLTLendBox(value = ErgCommons.MinBoxFee * 3, sigUSDAmount = goal, lenderAddress = dummyAddress)
    ...
    "convert to repayment box" in {
      client.getClient.execute(implicit ctx => {
        ...
        val sltLendToRepaymentTx: SLTLendToRepaymentTx = SLTLendToRepaymentTx(inputBoxes)
        ...
        assert(outSLTServiceBox.tokens(1).getValue - 1 == wrappedSLTServiceBox.tokens(1).getValue)
        assert(outSLTServiceBox.tokens(2).getValue + 1 == wrappedSLTServiceBox.tokens(2).getValue)
        assert(outRepaymentBox.fundingInfoRegister.equals(wrappedSLTLendBox.fundingInfoRegister))
        assert(outRepaymentBox.lendingProjectDetailsRegister.equals(wrappedSLTLendBox.lendingProjectDetailsRegister))
        assert(outRepaymentBox.singleLenderRegister.equals(wrappedSLTLendBox.singleLenderRegister))
        assert(outRepaymentBox.borrowerRegister.address.equals(wrappedSLTLendBox.borrowerRegister.address))
        assert(outRepaymentBox.loanTokenIdRegister.value.sameElements(wrappedSLTLendBox.loanTokenIdRegister.value))
      })
    }
  }
}
