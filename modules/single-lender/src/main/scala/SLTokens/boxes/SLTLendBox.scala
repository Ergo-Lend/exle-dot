...
import SLTokens.contracts.{SLTCreateLendBoxContract, SLTLendBoxContract}
import boxes.Box
import org.ergoplatform.appkit.{ErgoToken, ErgoType, InputBox}
import registers.{RegColl, RegVal}
import special.collection.Coll

case class SLTLendBox(inputBox: InputBox, contractData: SLTLendBoxContract) extends Box(inputBox) {
  val sltLendToken: ErgoToken = this.tokens.head
