...
case class SLELendBox(
  ...
  def this(inputBox: InputBox) = this(
    ...
    singleLenderRegister =
      if (inputBox.getRegisters.size() > 3)
  ...
  /**
    ...
    * @return
    */
  def getFundedOutputBox(
    ctx: BlockchainContext,
    txB: UnsignedTransactionBuilder
  ): OutBox = {
    // If lender is null, then it can't possibly be funded.
    if (singleLenderRegister.lendersAddress
          .getBytes()
          .deep == Array.emptyByteArray.deep) {
      //@todo create better exception
      throw new Exception(
        "Lender address is empty, but single lender box is funded."
      )
    }
