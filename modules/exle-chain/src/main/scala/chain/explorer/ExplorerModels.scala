...
object Models {
  ...
  case class Registers(
    R4: Option[RegisterData],
    R5: Option[RegisterData],
    R6: Option[RegisterData],
    R7: Option[RegisterData],
    R8: Option[RegisterData],
    R9: Option[RegisterData]
  ) {
  ...
  case class TokenData(
    ...
    override def toString: String =
      s"      Token(${id.toString.take(6)}...${id.toString.takeRight(6)} -> $amount | $name)"
  }

  case class ExtendedTokenData(
    id: ErgoId,
    amount: Long,
    index: Int,
    name: String,
    ...
    override def toString: String =
      s"      Token[$index](${id.toString.take(6)}...${id.toString.takeRight(6)} -> $amount | $name)"
  }
  ...
  case class Output(
    ...
    ergoTreeHex: String,
    address: Address,
    assets: Seq[ExtendedTokenData],
    registers: Registers,
    spendingTxId: Option[ErgoId],
    isOnMainChain: Boolean
  ) {
  ...
  case class FullBalance(
    confirmedBal: AddressBalance,
    unconfirmedBal: AddressBalance
  )
  ...
  case class HeaderData(
    ...
    nBits: Long,
    size: Int,
    extensnsionHash: String,
    powSolution: PowSolutionInfo,
    votes: Seq[Int]
  )
  ...
  case class BlockData(
    ...
    blockTxs: Seq[TransactionInfo1],
    blockExtensionInfo: BlockExtensionInfo,
    adProofs: String
  )

  case class BlockContainer(
    blockData: BlockData,
    blockReferenceData: BlockReferencesInfo
  )

  object RegisterData
      extends ExplorerConversion[AdditionalRegister, RegisterData] {
  ...
  object Registers extends ExplorerConversion[AdditionalRegisters, Registers] {
    ...
    def fromExplorer(regs: AdditionalRegisters): Registers = {
      ...
      Registers(
        RegisterData.fromOption(regMap.get("R4")),
        RegisterData.fromOption(regMap.get("R5")),
        RegisterData.fromOption(regMap.get("R6")),
        RegisterData.fromOption(regMap.get("R7")),
        RegisterData.fromOption(regMap.get("R8")),
        RegisterData.fromOption(regMap.get("R9"))
      )
    }
  }
  ...
  object HeaderData extends ExplorerConversion[HeaderInfo, HeaderData] {
    ...
    override def fromExplorer(exp: HeaderInfo): HeaderData =
      HeaderData(
        Helpers.toId(exp.getId),
        Helpers.toId(exp.getParentId),
        exp.getVersion,
        exp.getHeight,
        exp.getEpoch,
        exp.getDifficulty,
        exp.getAdProofsRoot,
        exp.getStateRoot,
        exp.getTransactionsRoot,
        exp.getTimestamp,
        exp.getNBits,
        exp.getSize,
        exp.getExtensionHash,
        exp.getPowSolutions,
        exp.getVotes.asScala.toSeq.map(_.toInt)
      )
  }
  ...
  object BlockData extends ExplorerConversion[FullBlockInfo, BlockData] {
    ...
    override def fromExplorer(exp: FullBlockInfo): BlockData =
      BlockData(
        HeaderData.fromExplorer(exp.getHeader),
        exp.getBlockTransactions.asScala.toSeq,
        exp.getExtension,
        exp.getAdProofs
      )
  }
  ...
  object BlockContainer
      extends ExplorerConversion[BlockSummary, BlockContainer] {
  ...
  object Output extends ExplorerConversion[OutputInfo, Output] {
    ...
    def fromExplorer(outputInfo: OutputInfo): Output =
      Output(
        Helpers.toId(outputInfo.getBoxId),
        Helpers.toId(outputInfo.getTransactionId),
        Helpers.toId(outputInfo.getBlockId),
        outputInfo.getValue,
        outputInfo.getIndex,
        outputInfo.getCreationHeight,
        outputInfo.getSettlementHeight,
        outputInfo.getErgoTree,
        Address.create(outputInfo.getAddress),
        ExtendedTokenData.fromExplorerList(outputInfo.getAssets),
        Registers.fromExplorer(outputInfo.getAdditionalRegisters),
        Option(outputInfo.getSpentTransactionId).map(str => ErgoId.create(str)),
        outputInfo.isMainChain
      )
  ...
  object Input extends ExplorerConversion[InputInfo, Input] {
    ...
    def fromExplorer(inputInfo: InputInfo): Input =
      Input(
        Helpers.toId(inputInfo.getBoxId),
        Helpers.toId(inputInfo.getOutputTransactionId),
        Helpers.toId(inputInfo.getOutputBlockId),
        inputInfo.getValue,
        inputInfo.getIndex,
        inputInfo.getSpendingProof,
        inputInfo.getOutputIndex,
        inputInfo.getErgoTree,
        Address.create(inputInfo.getAddress),
        ExtendedTokenData.fromExplorerList(inputInfo.getAssets),
        Registers.fromExplorer(inputInfo.getAdditionalRegisters)
      )
  ...
  object TransactionData
      extends ExplorerConversion[TransactionInfo, TransactionData] {

    def fromExplorer(txInfo: TransactionInfo): TransactionData =
      TransactionData(
        ErgoId.create(txInfo.getId),
        ErgoId.create(txInfo.getBlockId),
        txInfo.getInclusionHeight,
        txInfo.getTimestamp,
        txInfo.getIndex,
        txInfo.getNumConfirmations,
        Input.fromExplorerList(txInfo.getInputs),
        DataInput.fromExplorerList(txInfo.getDataInputs),
        Output.fromExplorerList(txInfo.getOutputs),
        txInfo.getSize
      )
  }
  ...
  object TokenData extends ExplorerConversion[TokenAmount, TokenData] {
    ...
    def fromExplorer(asset: TokenAmount): TokenData =
      TokenData(
        Helpers.toId(asset.getTokenId),
        asset.getAmount.longValue(),
        asset.getName,
        asset.getDecimals
      )
  }

  object ExtendedTokenData
      extends ExplorerConversion[AssetInstanceInfo, ExtendedTokenData] {
  ...
  object FullBalance extends ExplorerConversion[TotalBalance, FullBalance] {
    ...
    def fromExplorer(totalBalance: TotalBalance): FullBalance =
      FullBalance(
        AddressBalance.fromExplorer(totalBalance.getConfirmed),
        AddressBalance.fromExplorer(totalBalance.getUnconfirmed)
      )
  }
}
